    ...
    public class PerformikaPostModule
    {
        ...
        public async Task<List<Guid>> GetUids(string url, string workspaceId)
        {
            ...
            try
            {
                ...
                dynamic result = await _connector.GetJsonResultAsync($"{BaseUrl}{url}", HttpMethod.Post, _connector.CreateJsonContent(
                                workspaceId,
        ...
        {
            await LoadDict();
            List<EntityInfo> entityInfos = await GetEntityInfosAsync($"{BaseUrl}/gridview?definitionId=1be20f82-ff64-4b3a-bb38-920a8d7b2bfe&_=1615472618015",
                _connector.CreateJsonContent(
                    "47f5d6b9-3c0c-43f0-a99a-93692565b05e",
                    new List<string>
                    {
        ...
        /// <param name="pageSize">Количество получаемых данных</param>
        /// <param name="dt">Дата, начиная с которой необходимо получать значения</param>
        public async Task<List<RoadSection>> GetGetRoadSectionsAsync(int offset, int pageSize, DateTime dt = default)
        {
            await LoadDict();
            ...
            return entityInfos.Select(info =>
            {
                try
                {
                    ...
                    {
                        Id = info.Id,
                        Start = int.TryParse(info.Values.TryGetValue("49467883-626c-4f22-9c26-553fd0d0e523", out (string, string) start) ? start.Item1 : "0", out int s) ? s : 0,
                        StartAdd = int.TryParse(info.Values.TryGetValue("e9e6ca0d-aa40-49f1-b477-592c6d05aa95", out (string, string) startAdd) ? startAdd.Item1 : "0", out int sa) ? sa : 0,
                        Finish = int.TryParse(info.Values.TryGetValue("b477df52-5bbc-48ff-a5bf-63ed215c504d", out (string, string) finish) ? finish.Item1 : "0", out int f) ? f : 0,
                        FinishAdd = int.TryParse(info.Values.TryGetValue("d36044db-fc21-44cc-a5cd-e2947264458f", out (string, string) finishAdd) ? finishAdd.Item1 : "0", out int fa) ? fa : 0
                    };
